...
uint8_t num_anchors = NUM_ANCHORS;                          // the number of anchors
...
int32_t heights[NUM_ANCHORS] = {2750, 2000, 1900, 2350};              // anchor z-coordinates in mm
...
int32_t anchors_x[NUM_ANCHORS] = {0, 10000, 1000, 9000};              // anchor x-coorindates in mm
int32_t anchors_y[NUM_ANCHORS] = {0, 0, 7000, 8000};                  // anchor y-coordinates in mm
////////////////////////////////////////////////

boolean setAnchorsManualFlag = false;
...
void setup() {
  ...
  Serial.println("Waiting for manual anchor positions");
  ...
  while (Serial.available() > 0) {
    ...
    if(varCount > 3){
      if(varCount<6){
        anchors_x[varCount%num_anchors] = inputInt;
        setAnchorsManualFlag = true;
      }else{
        anchors_y[varCount%num_anchors] = inputInt;
      }
    }else{
      heights[varCount%num_anchors] = inputInt;
    }
    varCount+=1;
    delay(10);
  }
  if (setAnchorsManualFlag) {
    SetAnchorsManual();
  }

  //Debug echo for manual anchors
...
void printError(pos_error_t pos_error) {
